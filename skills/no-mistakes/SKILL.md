...
Recovery takes that head by fast-forward, by adopting a diverged preserved head proven to carry every local change - the ordinary result of the pipeline rebasing your commits onto a newer base - or, when your clean branch still equals the submitted head, by adopting an exact reviewed terminal head whose gate branch and run recovery ref both name it, after anchoring your pre-recovery head under `refs/no-mistakes/recover-local/<run>`.
