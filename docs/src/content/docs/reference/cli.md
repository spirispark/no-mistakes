...
That refusal is lifted only on positive proof, reported as `safety: blocked_pipeline_owned_head_lost` with `next_action.code: recover_custody`: Git itself must report the recorded head as missing from both the invoking worktree and a readable configured gate, no `refs/no-mistakes/recover*/<run>` evidence may survive in either, and the submitted head, any pushed head, and the gate's own branch head must all already be contained in the local branch.
`--recover` there returns custody with a stamp alone - no file, ref, or commit changes, and the commits recorded at the lost head cannot be restored by any command.
A behind, diverged, or unreadable relation, surviving recovery evidence, an unreadable or unconfigured gate, and a gate branch carrying commits the branch does not contain all keep the manual refusal.
