...
Under `--recover`, the possible worktree mutation is a strict fast-forward to the preserved pipeline head, an adoption of a preserved head proven to carry every local change, or an adoption of an exact reviewed terminal head when the clean branch still equals the submitted head and the review, terminal-head, gate-branch, and run recovery-ref evidence all agree, all after relation-specific preservation checks.
...
Without positive evidence that the submitted head stayed unchanged, custody is not guessed away. Missing or conflicting evidence and genuinely divergent import cases require manual reconciliation instead of advertising a recovery that will refuse; a dirty worktree refuses default recovery with explicit choices.
...
Terminalization pins a verified unpublished pipeline head under a run-specific recovery ref, so recovery does not require the gate branch itself to have advanced.
