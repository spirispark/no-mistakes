...
Under `--recover`, the possible worktree mutation is a strict fast-forward to the preserved pipeline head, an adoption of a preserved head proven to carry every local change, or an adoption of an exact reviewed terminal head when the clean branch still equals the submitted head and the review-approved-head, terminal-head, gate-branch, and exact run recovery-ref evidence all agree, all after relation-specific preservation checks.
...
A run that goes terminal (cancelled, failed, or completed without a push stage) after moving the pipeline head leaves the branch `pipeline_owned`. Status offers `next_action.code: recover_custody` only when recovery can establish the same eligibility it will enforce from a clean worktree: an equal or ahead local head proves the source locally and can create the local anchor when the gate is unavailable, but any existing gate recovery ref must still match the recorded head; importing a missing preserved head requires either local ancestry, the content-preservation proof described below, or an exact run-specific gate anchor for the preserved pipeline head that recovery will import and prove before moving anything. Legacy commit evidence that the gate can compare locally may still be anchored before recovery continues. The eligible state reports `safety: blocked_pipeline_owned_recoverable`, the run's terminal `pipeline.status`, and the exact `submitted_head`/`current_head`/`relation` ownership facts.
...
Without positive evidence that the submitted head stayed unchanged, custody is not guessed away. Missing or conflicting evidence and unproven divergent import cases require manual reconciliation instead of advertising a recovery that will refuse; a dirty worktree refuses default recovery with explicit choices.
...
For clean equal or ahead worktrees where the preserved head is already locally reachable, recovery writes that anchor locally without requiring gate access. If the gate is available, an existing symbolic, non-commit, or mismatched recovery ref is conflicting evidence and recovery refuses without overwriting it.
For behind or diverged worktrees, recovery verifies the preserved head at the run-specific recovery ref in the local gate and fetches it into the anchor before moving or refusing. A reviewed submitted-head replacement additionally requires the local gate branch itself to name that same preserved head before recovery starts. Legacy recorded heads that remain available as unreferenced gate objects are anchored before recovery continues.
A clean behind worktree fast-forwards.
A diverged worktree is adopted only when the preserved head provably carries every local change, proven by an executable three-way merge whose result is exactly the preserved head's tree, or when the clean invoking branch is still exactly the submitted head and the reviewed terminal-head proof above names the preserved head.
The content proof covers a pipeline rebase onto a newer base without requiring the gate branch to advance to the preserved head.
Terminalization pins a verified unpublished pipeline head under a run-specific recovery ref, so recovery ordinarily does not require the gate branch itself to have advanced; the reviewed submitted-head replacement is the exception because the gate branch is part of that proof.
A recorded head that is missing is normally `safety: blocked_recover_preserved_head_missing` with `next_action.code: inspect_and_reconcile_manually`, because a head that cannot be read is not a head that is known to be gone.
That refusal is lifted on positive proof, reported as `safety: blocked_pipeline_owned_head_lost` with `next_action.code: recover_custody`. Two positive proofs reach that safety: Git itself reports the recorded head as missing from both the invoking worktree and a readable configured gate while the worktree is clean, no `refs/no-mistakes/recover*/<run>` evidence survives in either, and the submitted head, any pushed head, and the gate's own branch head are all already contained in the local branch; or the recorded head is still reachable as a commit object (e.g. a loose object in the worktree's git-dir) but no longer an ancestor of the worktree branch while the worktree is clean, the local branch contains the submitted head, the pushed head when one exists, and the gate's own branch head, and no worktree or gate `refs/no-mistakes/recover*/<run>` anchor contradicts the recorded head. The two cases are distinguished by the `error` field - the lost case names the recorded head as missing from the worktree and gate, the orphan-but-contained case names it as unreachable as an ancestor of the branch.
`--recover` there returns custody with a stamp alone - no file, ref, or commit changes. The lost case's commits cannot be restored by any command; the orphan-but-contained case's commits live on through the integrated local branch.
A behind, diverged, or unreadable relation in the lost case, a recorded head that is an ancestor or descendant of the local branch, a missing ancestor or recovery anchor in the orphan-but-contained case, surviving recovery evidence, an unreadable or unconfigured gate, and a gate branch carrying commits the branch does not contain all keep the manual refusal.
...
Any divergence that neither proof can decide - unlanded local commits, or a rebase whose fix rounds also rewrote your own lines without the exact reviewed submitted-head evidence - still refuses with the anchor named, because only escalation can tell a deliberate pipeline fix apart from a dropped change.
A dirty worktree refuses default recovery with explicit choices.
