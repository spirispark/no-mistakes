...
const branchSyncAgentGuidance = "Before a post-pipeline local commit or fresh run, follow the structured `branch_sync.next_action`. Run `no-mistakes axi sync` only when its code is `sync`; that guarded sync may be a strict fast-forward or a content-equivalent diverged advance that anchors the pre-sync head before moving the branch with reset semantics. Run `no-mistakes axi sync --recover` only when its code is `recover_custody`: either a terminal run left unpublished pipeline commits preserved in the local gate, or `safety` is `blocked_pipeline_owned_head_lost`, meaning that run's recorded pipeline head is no longer importable - either it no longer exists anywhere while this branch already contains every head it recorded, or it is still reachable as a loose object but no longer an ancestor of this branch while every head it recorded is already contained here - so the recovery only returns custody and changes no file or ref. A `user_owned` state means cancellation released the branch before changing the submitted head: the exact branch and head are yours, immediately usable, and no sync action is needed. Process blocked or pipeline-owned states instead of improvising reset, stash, merge, rebase, force, or branch replacement."
