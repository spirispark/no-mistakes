...
If instead ` + "`safety`" + ` is ` + "`blocked_pipeline_owned_head_lost`" + `, that run's recorded pipeline head is no longer importable - either it no longer exists in the worktree or the local gate and cannot be restored by any command, or it is still reachable as a loose object but no longer an ancestor of this branch while every head the run recorded is already contained in it; in both cases ` + "`--recover`" + ` only returns custody and changes no file or ref.
Recovery takes that head by fast-forward, by adopting a diverged preserved head proven to carry every local change - the ordinary result of the pipeline rebasing your commits onto a newer base - or, when your clean branch still equals the submitted head, by adopting an exact review-approved terminal head whose gate branch and exact run recovery ref both name it, after anchoring your pre-recovery head under ` + "`refs/no-mistakes/recover-local/<run>`" + `.
The content proof is deliberately narrow, so a rebase whose fix rounds also rewrote your own lines refuses unless the exact reviewed submitted-head proof applies: when nothing can prove the preserved head is the run's reviewed replacement for an unchanged local submission, the decision is yours.
...
A dirty worktree, or divergence that neither proof accepts, makes the recovery refuse with explicit choices; ` + "`--keep-local`" + ` keeps your current head while the preserved commits stay anchored under ` + "`refs/no-mistakes/recover/<run>`" + `.
