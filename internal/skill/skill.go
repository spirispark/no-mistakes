...
If instead ` + "`safety`" + ` is ` + "`blocked_pipeline_owned_head_lost`" + `, that run's recorded pipeline head no longer exists in the worktree or the local gate and cannot be restored by any command; the branch already contains every head the run recorded, so ` + "`--recover`" + ` there only returns custody and changes no file or ref.
